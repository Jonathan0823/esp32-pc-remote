...
export function useLayoutContext(): LayoutContext {
  ...
  if (!ctx)
    throw new Error("useLayoutContext must be used inside a Layout route");
  return ctx;
}
    ...
    return {
      ...
      uptime:
        typeof s?.uptime_s === "number"
          ? formatDuration(s.uptime_s)
          : STATIC_DEVICE.uptime,
      freeHeap:
        typeof s?.heap === "number"
          ? `${Math.round(s.heap / 1024)} KB`
          : STATIC_DEVICE.freeHeap,
      mqttStatus: espOnline ? "Connected" : "Disconnected",
      broker: BROKER_URL ? formatBroker(BROKER_URL) : STATIC_DEVICE.broker,
      signalQuality:
        typeof s?.rssi === "number"
          ? signalQuality(s.rssi)
          : STATIC_DEVICE.signalQuality,
          ...
          <div className="sidebar-footer-card">
            <Card size="sm">
              <CardHeader className="flex flex-row items-center gap-2">
                ...
                <CardTitle className="text-xs text-sidebar-foreground/70">
                  Connection
                </CardTitle>
              </CardHeader>
